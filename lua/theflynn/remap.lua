...
--springboot运行器
function get_sprint_boot_runner(profile,debug)
    local debug_param = ""
    if debug then
        debug_param = '-Dspring-boot.run.javaArguments="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=*:5005"'
    ...
    return 'gradle bootRun ' .. profile_param .. debug_param
end

function run_spring_boot(debug)
--    vim.cmd('term ' .. get_sprint_boot_runner('local', debug))
    vim.cmd('term ' .. get_sprint_boot_runner('', debug))
end
